!function(canvas, ctx = canvas.getContext("2d"), track = loop = null) {
    const tool = {
        draw: {
            get left() {
                ctx.lineWidth = 1;
        ...
        descriptions: {
            left: {
                ...
                get 15() {
                    return "Erase Physics ( " + tool.eraser.settings.physics + " )";
                },
                get 16() {
                    return "Erase Scenery ( " + tool.eraser.settings.scenery + " )";
                },
                get 17() {
                    return "Erase Powerups ( " + tool.eraser.settings.powerups + " )";
                }
            },
            right: {
                    ...
                    if (V[2] !== void 0) {
                        if (V[0]) {
                            ...
                            if (document.documentElement.offsetHeight <= window.innerHeight) {
                                ctx.strokeText(V[2], canvas.width - 36, 15 + 25 * V[1]);
                                ctx.fillText(V[2], canvas.width - 36, 15 + 25 * V[1]);
                            } else {
                                ctx.strokeText(V[2], canvas.width - 51, 15 + 25 * V[1]);
                                ctx.fillText(V[2], canvas.width - 51, 15 + 25 * V[1]);
                            }
                            ctx.textAlign = "left";
                        } else {
                            ctx.strokeText(V[2], 36, 15 + 25 * V[1]);
                            ctx.fillText(V[2], 36, 15 + 25 * V[1]);
                        }
                    }
                }
                if (this.Ab) {
    ...
    class Ride {
        constructor(t) {
            window.Game.track = track = this.track = new Track(t);
...
}(document.getElementById("canvas_rider"));
